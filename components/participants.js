...
export function getParticipants() {
  ...
  getDocs(
    ...
    snapshot.forEach((doc) => {
      ...
      div.innerHTML = `
      <p><strong>${data.firstname} ${data.lastname}</strong> <button pid="${doc.id}" class="participant-edit">Edit</button><button pid="${doc.id}" class="participant-del" pName="${data.firstname}-${data.lastname}">Delete</button></p>
      `
    ...
    Total entrance fee: £${priceToBePaid}.00
    `
  })
}
